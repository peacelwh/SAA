...
class Block(nn.Module):
    def __init__(self, dim, num_heads, head_dim_ratio=1., mlp_ratio=4., qkv_bias=False, qk_scale=None,
        ...
        if not attn_disabled:
            ...
            self.attn = Attention(dim, num_heads=num_heads, head_dim_ratio=head_dim_ratio,
...
class Visformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, init_channels=32, num_classes=64, embed_dim=384, depth=12, phase='pre',
        ...
        if phase== 'meta':
            ...
            prompt_shape = (1, 384)
            # self.visual_prompts = [torch.zeros(prompt_shape, requires_grad=True) for _ in range(4)]
            # self.visual_prompts = [torch.randn(prompt_shape, requires_grad=True) for _ in range(4)]
            self.visual_prompt = torch.randn(prompt_shape, requires_grad=True)
            self.visual_prompt_ = torch.randn(prompt_shape, requires_grad=True)


            self.prompt_weight1 = nn.Parameter(torch.tensor(0.3))
            prompt_shape1 = (1, 192)
            # self.visual_prompts1 = [torch.zeros(prompt_shape1, requires_grad=True) for _ in range(4)]
            # self.visual_prompts1 = [torch.randn(prompt_shape1, requires_grad=True) for _ in range(4)]
            self.visual_prompt1 = torch.randn(prompt_shape1, requires_grad=True)
            self.visual_prompt1_ = torch.randn(prompt_shape1, requires_grad=True)
    ...
    def forward(self, x, args):
        ...
        x_reshape = local_features.permute(0, 2, 3, 1)
        y_reshape = self.classifier_local(x_reshape)
        y_token = y_reshape.permute(0, 3, 1, 2) #teacher=(5,65,8,7)
        y_reshape1 = self.head(x_reshape)
        y_token1 = y_reshape1.permute(0, 3, 1, 2)   #teacher=(5,64,8,7)
        
        return logit, x.squeeze(), y_token, y_token1, x.view(x.size(0), -1)
       

    def forward_with_semantic_prompt(self, x, semantic_prompt, args):
        prompt = self.t2i(semantic_prompt)
        prompt1 = self.t2i1(semantic_prompt)
        prompt2 = self.t2i2(semantic_prompt)
        prompt3 = self.t2i3(semantic_prompt)
        ...
        stage_scale2=1.3
        for b in self.stage1:
            if np.absolute(stage - stage_scale1) < 1e-6:
                B, C, H, W = x.shape

                # diverse prompts fusion
                fusion_prompt2 = self.prompt_weight1 * self.visual_prompt1.cuda() + prompt2
                fusion_prompt3 = self.prompt_weight1 * self.visual_prompt1_.cuda() + prompt3

                # channel fusion
                context = x.view(B, C, -1).mean(-1)
                context = torch.cat([context, fusion_prompt3], dim=-1) 
                context = self.se_block1(context)    
                context = context - context.mean(dim=-1, keepdim=True)
                x = x + context.view(B, C, 1, 1)  
                
                # sptial fusion
                fusion_prompt2 = fusion_prompt2.view(B, C, 1, 1).repeat(1, 1, 1, W)
                x = torch.cat([x, fusion_prompt2], dim=2) 
        ...
        stage_scale=3.2
        for b in self.stage3:
            if np.absolute(stage - stage_scale) < 1e-6:
                B, C, H, W = x.shape

                fusion_prompt = self.prompt_weight * self.visual_prompt.cuda() + prompt
                fusion_prompt1 = self.prompt_weight * self.visual_prompt_.cuda() + prompt1

                context = x.view(B, C, -1).mean(-1)
                context = torch.cat([context, fusion_prompt1], dim=-1) 
                context = self.se_block(context)    
                context = context - context.mean(dim=-1, keepdim=True)
                x = x + context.view(B, C, 1, 1)  

                fusion_prompt = fusion_prompt.view(B, C, 1, 1).repeat(1, 1, 1, W)
                x = torch.cat([x, fusion_prompt], dim=2) 
